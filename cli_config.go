...
func cliConfigPath() (string, error) {
	...
	if isFlagPassed(cliConfigFlag) {
		log.Infof("Using Homelab CLI config path from command line flag: %s", *cliConfig)
	...
	log.Infof("Using default Homelab CLI config path: %s", path)
	return path, nil
}
